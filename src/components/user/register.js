...
import { createUser } from '../../redux/reducers/userSlice.js';
...
const Register = () => {
  ...
  const validateFormData = () => {
    ...
    if (password === '') {
      errors.push(
        <>
          <span style={{ color: 'red', fontWeight: 'bold' }}>Password </span>
          <span>is required</span>
        </>
      );
  ...
  return (
    <div className="flex justify-center items-center min-h-screen signin-container">
      <div className="bg-white shadow-lg rounded-lg p-4 max-w-md w-full signin-content">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-4">
            <button
              type="submit"
              className="w-full p-2 bg-lime-500 text-white rounded-md cursor-pointer"
            >
              Sign Up
            </button>
          </div>
      ...
      {isErrorDialogOpen && (
        <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-gray-700 bg-opacity-50">
          <div className="bg-white shadow-lg rounded-md p-4 max-w-md w-full">
            <h3 className="text-2xl text-center text-red-500 font-semibold mb-4">
              Error
            </h3>
